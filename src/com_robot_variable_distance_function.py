...
def rom_calc(pos_list, time_taken, key):
    in_rad = 0.16
    o_rad = 0.3
    ...
    shift = pre1 * time_taken * 0.1  # sets shift amount
...
def on_key_release(key):  # Everything happens when the button is released so it knows how long the button was pressed
    ...
    if key.char == 'w':  # moves to robot in the positive x-axis
        ...
        print(rom_calc(pos_list, time_taken, 'w'))
        status, data = niryo_one_client.shift_pose(RobotAxis.X, rom_calc(pos_list, time_taken, 'w'))
        if status is False:
            print("Error: " + data)
    ...
    if key.char == 's':  # moves the robot in the negative x-axis
        ...
        print(rom_calc(pos_list, time_taken, 's'))
        status, data = niryo_one_client.shift_pose(RobotAxis.X, rom_calc(pos_list, time_taken, 's'))
        if status is False:
            print("Error: " + data)
    ...
    if key.char == 'a':  # moves the robot in the positive y-axis
        ...
        print(rom_calc(pos_list, time_taken, 'a'))
        status, data = niryo_one_client.shift_pose(RobotAxis.Y, rom_calc(pos_list, time_taken, 'a'))
        if status is False:
            print("Error: " + data)
    ...
    if key.char == 'd':  # moves the robot in the negative y-axis
        ...
        print(rom_calc(pos_list, time_taken, 'd'))
        status, data = niryo_one_client.shift_pose(RobotAxis.Y, rom_calc(pos_list, time_taken, 'd'))
        if status is False:
            print("Error: " + data)
...
while True:
    ...
    with keyboard.Listener(on_press=on_key_press) as press_listener:  # The listener just records if a button
